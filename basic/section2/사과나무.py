...
def get_apples(N, farms):
    apples = 0
    for i in range(N // 2):
        left = N // (i + 2)
        apples = (
            apples
            + sum(farms[i][left : N - left])
            + sum(farms[N - i - 1][left : N - left])
        )
    apples = apples + sum(farms[N // 2])
...
for i in range(1, 6):
    ...
    apple_farm = []
    for i in range(N):
        apple_farm.append(list(map(int, input().split())))

    get_apples(N, apple_farm)
